...
double *bufSW, *bufNW, *bufSE, *bufNE;
enum buffers
{
    bufSW,
    bufSE,
...
int check_in_which_node(particle_t *p)
{
    if (p->x_pos > 0)
    {
        if (p->y_pos > 0)
...
void move_particle(particle_t *p, double step, node_t *new_root)
{
    ...
    p->node = NULL;
    if (p->x_pos < XMIN || // XMIN = root->xmin
        p->x_pos > XMAX ||
        p->y_pos < YMIN ||
        p->y_pos > YMAX)
    {
    ...
    {
        // here we must test wether we can insert it or not. I.e., are we in the good node or not.

        if (check_in_which_node(p) == comm_rank)
        {
            insert_particle(p, new_root);
        }
        else // we couldnt insert it. We must send it to new node
        {
            // allocating buffer
            bufSW = malloc(4 * sizeof(double) * root->children[comm_rank].n_particles);
            bufSE = malloc(4 * sizeof(double) * root->children[comm_rank].n_particles);
            bufNW = malloc(4 * sizeof(double) * root->children[comm_rank].n_particles);
            bufNE = malloc(4 * sizeof(double) * root->children[comm_rank].n_particles);
            switch (check_in_which_node(p))
            {
            case 0:
                bufSW[cur_SW + 0] = p->x_pos;
                bufSW[cur_SW + 1] = p->y_pos;
                bufSW[cur_SW + 2] = p->x_vel;
                bufSW[cur_SW + 3] = p->y_vel;
                cur_SW += 4;
                break;
            case 1:
                bufSE[cur_SE + 0] = p->x_pos;
                bufSE[cur_SE + 1] = p->y_pos;
                bufSE[cur_SE + 2] = p->x_vel;
                bufSE[cur_SE + 3] = p->y_vel;
                cur_SE += 4;
                break;
            case 2:
                bufNW[cur_NW + 0] = p->x_pos;
                bufNW[cur_NW + 1] = p->y_pos;
                bufNW[cur_NW + 2] = p->x_vel;
                bufNW[cur_NW + 3] = p->y_vel;
                cur_NW += 4;
                break;
            case 3:
                bufNE[cur_NE + 0] = p->x_pos;
                bufNE[cur_NE + 1] = p->y_pos;
                bufNE[cur_NE + 2] = p->x_vel;
                bufNE[cur_NE + 3] = p->y_vel;
                cur_NE += 4;
                break;
...
void all_move_particles(double step)
{
    ...
    move_particles_in_node(&root->children[comm_rank], step, new_root);
    for (int i = 0; i < 4; i++)
    {
        if (i != comm_rank)
        {
            // MPI_Isend(buffers[i], cur)
        }
    }
